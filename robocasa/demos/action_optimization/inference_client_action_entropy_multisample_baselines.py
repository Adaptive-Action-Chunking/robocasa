...
import cv2
from robocasa.demos.help_function import set_seed, PORT_MAP
import argparse
from datetime import datetime
import os
from robocasa.demos.results_logger import ResultLogger, VideoLogger
# from robocasa.demos.action_optimization.action_entropy_v1 import adaptive_chunk, LiveEntropyPlot
from robocasa.demos.action_optimization.variance_analysis import ActionVarianceViz
import json
...
def run_task():
    ...
    # files to save
    parser.add_argument("--out_path", type=str, default="/home/sangfor/Videos/robocasa_debug", help="path to save the videos")
    ...
    # algorithm related
    # parser.add_argument("--adaptive_chunk", type=lambda x:x.lower()=="true", default=False, help="whether to use adaptive chunk size")
    ...
    now = datetime.now()
    args_dict = vars(args)
    args.out_path = os.path.join(args.out_path + "_" + task, task + "_" + now.strftime("%Y%m%d_%H%M%S")) # add time to out_path
    if args.save_videos:
    ...
    for trial_id in range(num_trials):
        ...
        frames = []
        # if args.debug: 
        #     entropy_plotter = LiveEntropyPlot()
        ...
        while end_episode==False and trial_success==False:
            ...
            if args.save_videos and args.wait_frames:
                wait_frame = np.zeros((config["camera_heights"], config["camera_widths"]*3, 3), dtype=np.uint8)
            ...
            # print("chunk to execute:", chunk_to_execute)
            for i in range(chunk_to_execute):
                ...
                # break if reach simulation time limit
                if done:
                    end_episode = True
                    if args.save_logs:
                        lift_tag = 1 if lift_success else 0
